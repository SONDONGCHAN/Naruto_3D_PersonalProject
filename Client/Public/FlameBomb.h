...
class CFlameBomb : public CSkill
{
public:
	enum FLAMEBOMB_STATE { STATE_MAKING, STATE_DETECTING, STATE_HIT, STATE_FINISH, STATE_END };

private:
	CFlameBomb(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CFlameBomb(const CFlameBomb& rhs);
	virtual ~CFlameBomb() = default;
...
public:
	static CFlameBomb* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);

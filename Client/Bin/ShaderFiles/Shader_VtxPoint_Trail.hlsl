...
void GS_MAIN(point GS_IN In[1], inout TriangleStream<GS_OUT> DataStream)
{
    ...
    Out[1].vPosition = In[0].vPosition + float4(vRight * g_vfThickness.y, 0.f) + float4(vLook + normalize(vLook) * 0.01f, 0.f);
    Out[1].vPosition = mul(Out[1].vPosition, matVP);
    Out[1].vTexcoord = float2(1.f, 0.f);

    Out[2].vPosition = In[0].vPosition + float4(vRight * g_vfThickness.x, 0.f);
    Out[2].vPosition = mul(Out[2].vPosition, matVP);
    Out[2].vTexcoord = float2(1.f, 1.f);

    Out[3].vPosition = In[0].vPosition - float4(vRight * g_vfThickness.x, 0.f);

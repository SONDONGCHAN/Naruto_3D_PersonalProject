...
void CLevel_BossStage::Tick(_float fTimeDelta)
{
...
HRESULT CLevel_BossStage::Ready_Lights()
{
...
HRESULT CLevel_BossStage::Ready_Layer_Camera(const wstring& strLayerTag)
{
	...
	CameraDesc.Current_LEVEL = LEVEL_BOSS;

	if (FAILED(m_pGameInstance->Add_CloneObject(LEVEL_BOSS, strLayerTag, TEXT("Prototype_GameObject_Camera_Free"), &CameraDesc)))
		return E_FAIL;
...
HRESULT CLevel_BossStage::Ready_Layer_Map(const wstring& strLayerTag)
{
	_uint Map_Index = 1;

	if (FAILED(m_pGameInstance->Add_CloneObject(LEVEL_BOSS, strLayerTag, TEXT("Prototype_GameObject_Map"),&Map_Index)))
		return E_FAIL;

	return S_OK;
}

HRESULT CLevel_BossStage::Ready_LandObjects()
{
	CLandObject::LANDOBJ_DESC			LandObjDesc{};

	LandObjDesc.pMapTransform = dynamic_cast<CTransform*>(m_pGameInstance->Get_Component(LEVEL_BOSS, TEXT("Layer_Map"), g_strTransformTag));
	LandObjDesc.pCamera = dynamic_cast<CCamera_Free*>(m_pGameInstance->Get_GameObject(LEVEL_BOSS, TEXT("Layer_Camera")));
	LandObjDesc.Current_Level = LEVEL_BOSS;
...
HRESULT CLevel_BossStage::Ready_Layer_Player(const wstring& strLayerTag, CLandObject::LANDOBJ_DESC& GameObjDesc)
{
	if (FAILED(m_pGameInstance->Add_CloneObject(LEVEL_BOSS, strLayerTag, TEXT("Prototype_GameObject_Player"), &GameObjDesc)))
		return E_FAIL;

	return S_OK;
}

HRESULT CLevel_BossStage::Ready_Layer_Monster(const wstring& strLayerTag, CLandObject::LANDOBJ_DESC& GameObjDesc)
{
	if (FAILED(m_pGameInstance->Add_CloneObject(LEVEL_BOSS, strLayerTag, TEXT("Prototype_GameObject_Monster_LeafNinja"), &GameObjDesc)))
		return E_FAIL;
...
HRESULT CLevel_BossStage::Ready_Layer_Item(const wstring& strLayerTag)
{
	return S_OK;
}
...
HRESULT CLevel_BossStage::Ready_Layer_UI(const wstring& strLayerTag)
{
	return S_OK;
}
...
_bool CLevel_BossStage::Check_Can_NextLevel()
{
	_uint DeadCheck = { 0 };
...
CLevel_BossStage* CLevel_BossStage::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{

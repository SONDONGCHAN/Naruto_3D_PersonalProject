...
CBody_Player_Custom_Face::CBody_Player_Custom_Face(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CPartObject(pDevice, pContext)
{
}

CBody_Player_Custom_Face::CBody_Player_Custom_Face(const CBody_Player_Custom_Face& rhs)
	:CPartObject(rhs)
{
}

HRESULT CBody_Player_Custom_Face::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBody_Player_Custom_Face::Initialize(void* pArg)
{
	...
	{
		m_FaceModels[i]->Set_OffRootAnim(true);
		m_FaceModels[i]->Set_Animation(139, 0.f, true, 1.0f, false, false);
	}

	if (pBodyPlayerDesc->Current_LEVEL != LEVEL_CUSTOMROOM) {
		m_CurrentModel_Index = g_iPartsIndex[0];
		m_pCurrentModel = m_FaceModels[m_CurrentModel_Index];
	}
	else
		m_pCurrentModel = m_FaceModels[0];

	return S_OK;
}

void CBody_Player_Custom_Face::Priority_Tick(_float fTimeDelta)
{
}

void CBody_Player_Custom_Face::Tick(_float fTimeDelta)
{
}

void CBody_Player_Custom_Face::Late_Tick(_float fTimeDelta)
{
	Set_Animation_State(fTimeDelta);
	m_FaceModels[m_CurrentModel_Index]->Play_Animation(fTimeDelta, m_fAnimationPlayRatio);
...
HRESULT CBody_Player_Custom_Face::Render()
{
	if (FAILED(Bind_ShaderResources()))
		return E_FAIL;

	_uint	iNumMeshes = m_FaceModels[m_CurrentModel_Index]->Get_NumMeshes();
...
void CBody_Player_Custom_Face::Set_Animation_State(_float fTimeDelta)
{
...
void CBody_Player_Custom_Face::Set_Model(_bool UpDown)
{
	if (UpDown)
	{
	...
	g_iPartsIndex[0] = m_CurrentModel_Index;
	m_pCurrentModel = m_FaceModels[m_CurrentModel_Index];
}

CModel* CBody_Player_Custom_Face::Get_CurrentModel()
{
	return __super :: Get_CurrentModel();
}

HRESULT CBody_Player_Custom_Face::Add_Component(LEVEL Current_LEVEL)
{
...
HRESULT CBody_Player_Custom_Face::Add_Models(LEVEL Current_LEVEL)
{
	CModel* m_pModel_Byaku = { nullptr };
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Player_Custom_Face_Byaku"),
		TEXT("Com_Model_Face_Byaku"), reinterpret_cast<CComponent**>(&m_pModel_Byaku))))
		return E_FAIL;
	m_FaceModels.push_back(m_pModel_Byaku);
	m_Num_Models++;

	CModel* m_pModel_Mask = { nullptr };
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Player_Custom_Face_Mask"),
		TEXT("Com_Model_Face_Mask"), reinterpret_cast<CComponent**>(&m_pModel_Mask))))
		return E_FAIL;
	m_FaceModels.push_back(m_pModel_Mask);
	m_Num_Models++;

	return S_OK;
}

HRESULT CBody_Player_Custom_Face::Bind_ShaderResources()
{
...
CBody_Player_Custom_Face* CBody_Player_Custom_Face::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBody_Player_Custom_Face* pInstance = new CBody_Player_Custom_Face(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CBody_Player_Custom_Face");
...
CGameObject* CBody_Player_Custom_Face::Clone(void* pArg)
{
	CBody_Player_Custom_Face* pInstance = new CBody_Player_Custom_Face(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CBody_Player_Custom_Face");
		Safe_Release(pInstance);
	}
...
void CBody_Player_Custom_Face::Free()
{
	for (auto pModel : m_FaceModels)
		Safe_Release(pModel);
	m_FaceModels.clear();

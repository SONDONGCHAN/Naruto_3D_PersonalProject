...
CBody_Player_Custom_Upper::CBody_Player_Custom_Upper(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CPartObject(pDevice, pContext)
{
}

CBody_Player_Custom_Upper::CBody_Player_Custom_Upper(const CBody_Player_Custom_Upper& rhs)
	:CPartObject(rhs)
{
}

HRESULT CBody_Player_Custom_Upper::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBody_Player_Custom_Upper::Initialize(void* pArg)
{
	...
	{
		m_UpperModels[i]->Set_OffRootAnim(true);
		m_UpperModels[i]->Set_Animation(139, 0.f, true, 1.0f, false, false);

	}
	if (pBodyPlayerDesc->Current_LEVEL != LEVEL_CUSTOMROOM) {
		m_CurrentModel_Index = g_iPartsIndex[3];
		m_pCurrentModel = m_UpperModels[m_CurrentModel_Index];
	}
	else
		m_pCurrentModel = m_UpperModels[0];

	return S_OK;
}

void CBody_Player_Custom_Upper::Priority_Tick(_float fTimeDelta)
{
}

void CBody_Player_Custom_Upper::Tick(_float fTimeDelta)
{
}

void CBody_Player_Custom_Upper::Late_Tick(_float fTimeDelta)
{
	Set_Animation_State(fTimeDelta);
	m_UpperModels[m_CurrentModel_Index]->Play_Animation(fTimeDelta, m_fAnimationPlayRatio);
...
HRESULT CBody_Player_Custom_Upper::Render()
{
	if (FAILED(Bind_ShaderResources()))
		return E_FAIL;

	_uint	iNumMeshes = m_UpperModels[m_CurrentModel_Index]->Get_NumMeshes();
...
void CBody_Player_Custom_Upper::Set_Animation_State(_float fTimeDelta)
{
...
void CBody_Player_Custom_Upper::Set_Model(_bool UpDown)
{
	if (UpDown)
	{
	...
	g_iPartsIndex[3] = m_CurrentModel_Index;
	m_pCurrentModel = m_UpperModels[m_CurrentModel_Index];
}

CModel* CBody_Player_Custom_Upper::Get_CurrentModel()
{
	return m_pCurrentModel;
}

HRESULT CBody_Player_Custom_Upper::Add_Component(LEVEL Current_LEVEL)
{
...
HRESULT CBody_Player_Custom_Upper::Add_Models(LEVEL Current_LEVEL)
{
	CModel* m_pModel_Vest = { nullptr };
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Player_Custom_Upper_Vest"),
		TEXT("Com_Model_Upper_Vest"), reinterpret_cast<CComponent**>(&m_pModel_Vest))))
		return E_FAIL;
	m_UpperModels.push_back(m_pModel_Vest);
	m_Num_Models++;

	CModel* m_pModel_Jacket = { nullptr };
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Player_Custom_Upper_Jacket"),
		TEXT("Com_Model_Upper_Jacket"), reinterpret_cast<CComponent**>(&m_pModel_Jacket))))
		return E_FAIL;
	m_UpperModels.push_back(m_pModel_Jacket);
	m_Num_Models++;
...
HRESULT CBody_Player_Custom_Upper::Bind_ShaderResources()
{
...
CBody_Player_Custom_Upper* CBody_Player_Custom_Upper::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBody_Player_Custom_Upper* pInstance = new CBody_Player_Custom_Upper(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CBody_Player_Custom_Upper");
...
CGameObject* CBody_Player_Custom_Upper::Clone(void* pArg)
{
	CBody_Player_Custom_Upper* pInstance = new CBody_Player_Custom_Upper(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CBody_Player_Custom_Upper");
		Safe_Release(pInstance);
	}
...
void CBody_Player_Custom_Upper::Free()
{
	for (auto pModel : m_UpperModels)
		Safe_Release(pModel);
	m_UpperModels.clear();

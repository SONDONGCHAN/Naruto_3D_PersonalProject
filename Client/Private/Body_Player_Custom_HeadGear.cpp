...
CBody_Player_Custom_HeadGear::CBody_Player_Custom_HeadGear(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CPartObject(pDevice, pContext)
{
}

CBody_Player_Custom_HeadGear::CBody_Player_Custom_HeadGear(const CBody_Player_Custom_HeadGear& rhs)
	:CPartObject(rhs)
{
}

HRESULT CBody_Player_Custom_HeadGear::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBody_Player_Custom_HeadGear::Initialize(void* pArg)
{
	...
	{
		m_HeadGearModels[i]->Set_OffRootAnim(true);
		m_HeadGearModels[i]->Set_Animation(139, 0.f, true, 1.0f, false, false);
	}

	if (pBodyPlayerDesc->Current_LEVEL != LEVEL_CUSTOMROOM) {
		m_CurrentModel_Index = g_iPartsIndex[1];
		m_pCurrentModel = m_HeadGearModels[m_CurrentModel_Index];
	}
	else
		m_pCurrentModel = m_HeadGearModels[0];

	return S_OK;
}

void CBody_Player_Custom_HeadGear::Priority_Tick(_float fTimeDelta)
{
}

void CBody_Player_Custom_HeadGear::Tick(_float fTimeDelta)
{
}

void CBody_Player_Custom_HeadGear::Late_Tick(_float fTimeDelta)
{
	Set_Animation_State(fTimeDelta);
	m_HeadGearModels[m_CurrentModel_Index]->Play_Animation(fTimeDelta, m_fAnimationPlayRatio);
...
HRESULT CBody_Player_Custom_HeadGear::Render()
{
	if (FAILED(Bind_ShaderResources()))
		return E_FAIL;

	_uint	iNumMeshes = m_HeadGearModels[m_CurrentModel_Index]->Get_NumMeshes();
...
void CBody_Player_Custom_HeadGear::Set_Animation_State(_float fTimeDelta)
{
...
void CBody_Player_Custom_HeadGear::Set_Model(_bool UpDown)
{
	if (UpDown)
	{
	...
	g_iPartsIndex[1] = m_CurrentModel_Index;
	m_pCurrentModel = m_HeadGearModels[m_CurrentModel_Index];
}

CModel* CBody_Player_Custom_HeadGear::Get_CurrentModel()
{
	return __super ::Get_CurrentModel();
}

HRESULT CBody_Player_Custom_HeadGear::Add_Component(LEVEL Current_LEVEL)
{
...
HRESULT CBody_Player_Custom_HeadGear::Add_Models(LEVEL Current_LEVEL)
{
	CModel* m_pModel_AllBack = { nullptr };
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Player_Custom_HeadGear_AllBack"),
		TEXT("Com_Model_HeadGear_AllBack"), reinterpret_cast<CComponent**>(&m_pModel_AllBack))))
		return E_FAIL;
	m_HeadGearModels.push_back(m_pModel_AllBack);
	m_Num_Models++;

	CModel* m_pModel_HatTurban = { nullptr };
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Player_Custom_HeadGear_HatTurban"),
		TEXT("Com_Model_HeadGear_HatTurban"), reinterpret_cast<CComponent**>(&m_pModel_HatTurban))))
		return E_FAIL;
	m_HeadGearModels.push_back(m_pModel_HatTurban);
	m_Num_Models++;

	/*CModel* m_pModel_HalfUpBun = { nullptr };
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Player_Custom_HeadGear_HalfUpBun"),
		TEXT("Com_Model_HeadGear_HalfUpBun"), reinterpret_cast<CComponent**>(&m_pModel_HalfUpBun))))
		return E_FAIL;
	m_HeadGearModels.push_back(m_pModel_HalfUpBun); 
	m_Num_Models++;*/

	return S_OK;
}

HRESULT CBody_Player_Custom_HeadGear::Bind_ShaderResources()
{
...
CBody_Player_Custom_HeadGear* CBody_Player_Custom_HeadGear::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBody_Player_Custom_HeadGear* pInstance = new CBody_Player_Custom_HeadGear(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CBody_Player_Custom_HeadGear");
...
CGameObject* CBody_Player_Custom_HeadGear::Clone(void* pArg)
{
	CBody_Player_Custom_HeadGear* pInstance = new CBody_Player_Custom_HeadGear(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CBody_Player_Custom_HeadGear");
		Safe_Release(pInstance);
	}
...
void CBody_Player_Custom_HeadGear::Free()
{
	for (auto pModel : m_HeadGearModels)
		Safe_Release(pModel);
	m_HeadGearModels.clear();
